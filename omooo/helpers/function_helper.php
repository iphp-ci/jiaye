...
function dr_down_server($name, $value, $dirname = APP_DIR) {
    ...
    $file = str_replace('member/uploadfile/', '', $file['_attachment']);
    $server = array();
...
function dr_space_info($uid, $cache = 10000) {
    ...
    if (!$data) {
        ...
        $ci->set_cache_data('space-info-' . $uid, $data, $cache ? $cache : 10000);
    }
...
function dr_sns_info($uid, $cache = 10000) {
    ...
    if (!$data) {
        ...
        $ci->set_cache_data('sns-info-'.$uid, $data, $cache ? $cache : 10000);
    }
...
function dr_baidu_map($value, $zoom = 5, $width = 600, $height = 400) {
	...
	drawPoints();
	function drawPoints(){
		var myIcon = new BMap.Icon("' . SITE_URL . 'omooo/statics/images/mak.png", new BMap.Size(27, 45));
...
function dr_show_stars($num, $starthreshold = 4) {
    ...
    for ($i = 3; $i > 0; $i--) {
        ...
        for ($j = 0; $j < $numlevel; $j++) {
            $str.= '<img align="absmiddle" src="' . SITE_URL . 'omooo/statics/images/star_level' . $i . '.gif" ' . $alt . ' />';
        }
    }
...
function get_attachment($id) {
    ...
    $info = $ci->db
               ...
               ->where('id', (int)$id)
               ->limit(1)
               ->get('attachment_'.(int)$data['tableid'])
               ->row_array();
    ...
    // 远程图片
    if ($info['remote']) {
        $url = $ci->get_cache('siteinfo', SITE_ID, 'remote', $info['remote'], 'SITE_ATTACH_URL');
        $info['attachment'] = $url.'/'.$info['attachment'];

    }
    $info['_attachment'] = $info['attachment'];
    $ci->set_cache_data("attachment-{$id}", $info, 3600); // 保存附件缓存
...
function dr_thumb2($img, $width = 100, $height = 100, $autocut = 1) {
    ...
    if (!$img) {
        return SITE_URL . 'omooo/statics/images/nopic.gif';
    }

    // 当图片是附件id或者本站图片时才进行缩略图处理
...
function dr_thumb($img, $width = NULL, $height = NULL, $water = 1) {
    ...
    if (is_numeric($img)) { // 表示附件id
        ...
        $thumb = "index.php?c=api&m=thumb&id=$img&width=$width&height=$height&water=$water";
        $tfile = 'uploadfile/thumb/'.md5($thumb).'.jpg';

        if (is_file(FCPATH.'member/'.$tfile)) {
            return MEMBER_URL.$tfile;
        }

        $info = get_attachment($img);
        if ($width || $height) {
            return MEMBER_URL.$thumb;
        }
    ...
    return $img ? $img : SITE_URL.'omooo/statics/images/nopic.gif';
}
...
function dr_down_file($id) {
    ...
    if (is_numeric($id)) { // 表示附件id
        ...
        if ($info) {
            return MEMBER_URL."index.php?c=api&m=file&id=$id";
        }
    }
...
function get_module($dirname, $siteid = SITE_ID) {
    ...
    $dirname = $dirname == 'APP_DIR' ? APP_DIR : $dirname;
...
function dr_sns_feed($id, $cache = 3600) {
    ...
    if (!$data) {
        ...
        $ci->set_cache_data('sns-feed-'.$id, $data, $cache ? $cache : 3600);
    }
